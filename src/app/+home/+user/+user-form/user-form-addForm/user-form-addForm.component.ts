...
@Component({
	...
	styleUrls: ['./user-form-addForm.component.scss'],
...
export class UserFormAddFormComponent implements OnInit{ 
	...
	Statue:any
	returnTo(){
		this.router.navigate(['../../../../user-form-vipEdit',this.ID,this.Name],{relativeTo:this.activatedRoute}); 
	}
default:any={
		groupGrpNm:'',
		groupTp:'9',
		...
		let listState= <HTMLInputElement[]><any>document.getElementsByName("user");				
			for(var i =0;i<listState.length;i++){
		 		if(listState[i].checked == true){
		 			this.checkGroup.push({groupId:this.group[i].groupGrpid,serviceId:this.ID,role:this.Statue});
...
// 添加
	addGroupUser(changeGroup:any){
		this.userFormService.addServiceGroup(this.checkGroup).subscribe(
			result => {  
							if(result.status==0){
								alert("添加成功！")
